...
void simulate(uchar3 *ptr, int tick, int w, int h)
{
	...
	*/
	...
	{
		err = cudaMalloc(&d_particlePosition, sizeof(float) * (800 * 800));
		if (err != cudaSuccess)
		{
		...
		err = cudaMalloc(&d_particleVelocity, sizeof(float) * (800 * 800));
		if (err != cudaSuccess)
		{
